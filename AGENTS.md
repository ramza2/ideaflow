...
- Admin (Step 11): `SystemSetting` (4 known boolean keys), `/api/v1/admin/*` (SYSTEM_ADMIN + password changed), user management, integration config read + LLM/web-search/embedding diagnostic tests. Connection secrets stay in ENV only (`api_key_configured` bool in API). Global LLM/web-search policy enforced in AI/research services; workspace responses include `effective_allow_*`.
- Settings polish (Step 17.5): Workspace (`/workspace/*`) vs user settings (`/settings/*`) IA separation; `PATCH /auth/me` name-only self-update; Admin Embedding diagnostics; environment-managed provider config remains read-only; dead placeholder controls removed from Home/AI Input.
